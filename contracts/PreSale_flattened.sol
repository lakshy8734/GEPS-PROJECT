...
contract GEPSPresale is Ownable, ReentrancyGuard {
    ...
    uint256 public constant STAGE_DURATION = 40 days;
    uint256 public constant CLAIM_DELAY = 2 days;
    bool public unsoldGEPSTransferred = false;
    ...
    function buyGEPS(uint256 amount, string memory currency) external payable nonReentrant {
        ...
        if (stages[currentStage].endTime <= block.timestamp) {
            if (currentStage == 8) {
                ...
                presaleEndTime = stages[currentStage].endTime;
                emit PresaleEnded();
            } else {
                advanceStage();
            }
        }
        uint256 gepsAmount = uint(calculateGEPS(amount, currency));
        if (keccak256(bytes(currency)) == keccak256(bytes("BNB"))) {
            require(msg.value >= amount, "Insufficient BNB sent");
            presaleWallet.transfer(amount);
        } else if (keccak256(bytes(currency)) == keccak256(bytes("BUSD"))) {
            require(
                busd.transferFrom(msg.sender, presaleWallet, amount),
        ...
        allocateGEPSs(msg.sender, gepsAmount);
        emit GEPSsPurchased(msg.sender, gepsAmount, currency);
    ...
    function claimGEPSs() external onlyAfterPresale nonReentrant {
        require(
